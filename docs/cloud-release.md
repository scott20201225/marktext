...
MarkText ships a GitHub Actions build/release matrix for all desktop targets we currently package in CI:
...
Use the `Release MarkText` workflow in one of two ways:

1. Push a semver tag like `v0.20.0-beta.1`.
2. Or run `Release MarkText` manually and provide an existing semver tag.
